...
const batch_func = `  const [scanTarget, setScanTarget] = useState<'starter' | 'substitute' | 'staff'>('starter');
...
const new_team_name_code = `<CardTitle className="text-lg font-black uppercase italic">{teamNames[side]}</CardTitle>
                      <DropdownMenu>
...
content = content.split(`<CardTitle className="text-lg font-black uppercase italic">{teamNames[side]}</CardTitle>\n                    </div>`).join(new_team_name_code);
content = content.split(`<CardTitle className="text-lg font-black uppercase italic">{teamNames[side]}</CardTitle>\r\n                    </div>`).join(new_team_name_code);

const hidden_input = `<input id="camera-scan" type="file" accept="image/*" capture="environment" className="hidden" onChange={handleScanBatch} disabled={isScanning} />`;
